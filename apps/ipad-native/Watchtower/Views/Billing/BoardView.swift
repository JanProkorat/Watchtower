...
/// iPad port of the iPhone `BoardView` — same read-only Kanban read of
/// `buildBoard(tasks:worklogs:projectId:)` over the shared billing dataset,
/// with the project filter bound to `RecordsFeature.boardProjectId`. iPhone
/// has no Mac bridge, so the iPad-only actions (re-sync from Jira, upload
/// worklogs — `BoardActions` in the React version) are dropped entirely
...
private struct BoardCardTile: View {
    ...
    /// `"<logged> h"` or `"<logged> / <estimate> h"`. Mirrors the iPhone
    /// `BoardView`'s `timeLine` (in turn `BoardView.tsx`'s `timeLine()`); the
    /// combined form composes two bare (unit-less) hour figures with a
    /// single trailing " h" so it doesn't repeat the unit.
