...
/// Native port of `packages/module-timetracker/src/billing/reports/EarningsSummaryPanel.tsx`.
/// 2×2 stat tiles + a per-project earnings breakdown (tap a row to navigate).
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if !summary.perProject.isEmpty {
                GlassCard {
                    VStack(spacing: 12) {
                        ForEach(summary.perProject, id: \.projectId) { project in
                            Button {
